...
func main() {
	asset, err := OpenAsset("EGAGRAPH.C3D", "EGAHEAD.C3D", "EGADICT.C3D")
	if err != nil {
		panic(err)
	}
	...
	for i := 456; i < 476; i++ {
		...
		err = ioutil.WriteFile(fmt.Sprintf("text/%v.dat", i), data, 0666)
		if err != nil {
			panic(err)
		}
	}
